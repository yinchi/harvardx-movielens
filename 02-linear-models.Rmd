...
$$
\text{SE}_{\mathcal{V}} =
\sum_{(u,i)\in\mathcal{V}} \left[Y_{u,i} - \mu - \sum_j \beta_{j;u,i} \right]^2
$$
where $\mathcal{V}$ represents the validation set of observed movie ratings.
This can be thought of as a $\mathcal{V}$-based approximation of
$$
\text{SE} = \left\Vert Y - \mu - \sum_j \beta_j \right\Vert_{2,2}^2.
$$

The estimated value of $Y$ for $(u,v) \notin \mathcal{T}$ is
$$
...
$$
The best estimate $\hat{\mu}$ of $\mu$ is the mean of all ratings in `edx`, or:
...
This model gives the following RMSE values when applied to the `validation` set:
...
$$
The least-squares estimate $\hat{b}_{1;i}$ of $b_{1;i}$ is the mean of
...
all ratings where $g(i) = g$. The following code computes
...
$$
\text{SE}_{\mathcal{V}} =
\sum_{(u,i)\in\mathcal{V}} \left[y_{u,i} - \mu - b_{1;i} - b_{2;u} - b_{3;g(i)} - f(t_{u,i})\right]^2.
$$
In matrix form, the above is an validation-set-based approximation of
$$
\text{SE} = \left\Vert Y - \mu_t - \sum_j \beta_j \right\Vert_{2,2}^2.
$$
...
$$
\text{SE}_{\mathcal{V}} = \sum_{(u,i)\in\mathcal{V}} \left[y_{u,i} - \mu - b_{1;i} - b_{2;u} - b_{3;g(i)} - f(t_{u,i})\right]^2,
$$
in this section we add a penalty term such that the new expression to minimize is as follows:
$$
\text{SE}_{\mathcal{V}} + \lambda\left(\sum_i b_{1;i}^2 + \sum_u b_{2;u}^2 + \sum_g b_{3;g}^2 \right).
$$
In matrix form, the above is an validation-set-based approximation of
$$
\left\Vert Y - \mu_t - \sum_j \beta_j \right\Vert_{2,2}^2 + \lambda\sum_j\left\Vert b_j\right\Vert_2^2.
$$

To perform regularization, we first split `edx` into a training and test set:
