...
There are two reasons why `cmfrec` outperforms our method in this project.
Firstly, the `cmfrec` algorithms *update* user and movie biases during each iteration, rather than the
static method used here.  The benchmarks shown in @cmfrec-benchmarks show that such static methods
generally perform worse than methods with iterative bias updates. The methods in `cmfrec` also use
alternating least squares (ALS) by default rather than the gradient descent method used here,
which improves numerical stability.  Another benefit of ALS is the possibility of
massive paralellization [@Koren2009].
