...
error (RMSE):
$$
\text{RMSE}_{\mathcal{V}} = \sqrt{\frac{1}{N}\sum_{(u,i)\in\mathcal{V}} \left(y_{u,i} - \hat{y}_{u,i}\right)^2}
$$
where $y$ denotes the true values of movie ratings in the validation set $\mathcal{V}$,
...
$$
\text{SE}_{\mathcal{V}} = \sum_{(u,i)\in\mathcal{V}} \left(y_{u,i} - \hat{y}_{u,i}\right)^2.
$$
In matrix form, this can be thought of as a $\mathcal{V}$-based approximation of the square of the
$L_{2,2}$ or Frobenius norm, i.e.,
$$
